#!/usr/bin/env python3
"""
Real 2025 MLB Data Collector
Uses MLB Stats API to collect current 2025 season data
"""
...
class Real2025DataCollector:
    """Collects actual 2025 MLB data from live APIs"""
    ...
    def __init__(self, db_path='data/mlb_predictions.db'):
        ...
        Path('data/raw/2025').mkdir(parents=True, exist_ok=True)
    ...
    def _save_game_data(self, game_data, date_str):
        ...
        try:
            ...
            # Extract game info
            game_info = game_data.get('gameId', {})
            game_pk = game_info.split('/')[-1] if isinstance(game_info, str) else str(game_data.get('game', {}).get('pk', ''))
            ...
            conn.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            ''', (game_pk, date_str, home_team, away_team, venue, "19:00"))
    ...
    def test_api_connection(self):
        ...
        try:
            ...
            if total_games > 0:
                sample_game = data['dates'][0]['games'][0]
...
def main():
    ...
    if len(sys.argv) >= 3:
        ...
        # Default: collect recent 2025 data
